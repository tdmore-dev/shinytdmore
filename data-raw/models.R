...
#________________________________________________________________________________________
#----                       Model: D0_14_Troughs_1cpt_BASE_TSFD                      ----
#________________________________________________________________________________________
...
m1Code <- thetaValues %>% paste("
                                ...
                                V = exp(log(V_pop) + eta_TxNR_V );
                                k0 = exp(log(k0_pop) + eta_TxNR_k0 );
                                kplus = exp(log(kplus_pop) + eta_TxNR_kplus );
                                kk = exp(log(kk_pop) + eta_TxNR_kk );
...
D0_14_Troughs_1cpt_BASE_TSFD <- RxODE::RxODE(m1Code) %>% tdmore(
  parameters=rownames(omega),
  omega=omega,
  iov=omegaMonolix %>% filter(GAMMA) %>% pull(etaName), 
  res_var=list( errorModel(var="Cwb", 
                           add=monolixValues %>% filter(parameter=="a") %>% pull(value), 
...
#________________________________________________________________________________________
#----                     Model: D0_14_Troughs_1cpt_BASE_TSFD_MPC                    ----
#________________________________________________________________________________________
...
D0_14_Troughs_1cpt_BASE_TSFD_MPC <- RxODE::RxODE(m1Code) %>% tdmore(
  parameters=rownames(omega),
  omega=omega,
  iov=rownames(omega), 
  res_var=list( errorModel(var="Cwb", 
